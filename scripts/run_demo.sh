#!/bin/bash
# AMPM Demo Meet Bot - Type questions in terminal
# Usage: ./scripts/run_demo.sh <meeting_url>
# Example: ./scripts/run_demo.sh "https://meet.google.com/abc-defg-hij"
...
if [ -z "$CEREBRAS_API_KEY" ] && [ -z "$OPENAI_API_KEY" ]; then
    echo "Error: Missing API key. Set CEREBRAS_API_KEY or OPENAI_API_KEY in .env"
    exit 1
fi
...
echo "Starting AMPM Demo Meet Bot..."
